...
open class FloatRatingView: UIView {
    ...
    */
    ...
    fileprivate func removeImageViews() {
        ...
        for iVar in 0..<emptyImageViews.count {
            var imageView = emptyImageViews[iVar]
            imageView.removeFromSuperview()
            imageView = fullImageViews[iVar]
            imageView.removeFromSuperview()
        }
    ...
    fileprivate func refresh() {
        for iVar in 0..<fullImageViews.count {
            let imageView = fullImageViews[iVar]

            if rating >= Float(iVar+1) {
                imageView.layer.mask = nil
                imageView.isHidden = false
            } else if rating > Float(iVar) && rating < Float(iVar+1) {
                // Set mask layer for full image
                let maskLayer = CALayer()
                maskLayer.frame = CGRect(x: 0, y: 0, width: CGFloat(rating-Float(iVar))*imageView.frame.size.width, height: imageView.frame.size.height)
    ...
    fileprivate func updateLocation(_ touch: UITouch) {
        ...
        for iVar in stride(from: (maxRating-1), through: 0, by: -1) {
            let imageView = emptyImageViews[iVar]
            ...
            if imageView.point(inside: newLocation, with: nil) && (floatRatings || halfRatings) {
                ...
                newRating = Float(iVar) + decimalNum
                if halfRatings {
                    newRating = Float(iVar) + (decimalNum > 0.75 ? 1 : (decimalNum > 0.25 ? 0.5 : 0))
                }
            } else {
                // Whole rating
                newRating = Float(iVar) + 1.0
            }
            break
        }
    ...
    override open func layoutSubviews() {
        ...
        for iVar in 0..<maxRating {
            let imageFrame = CGRect(x: iVar == 0 ? 0 : CGFloat(iVar)*(imageXOffset+imageViewSize.width), y: 0, width: imageViewSize.width, height: imageViewSize.height)
