...
@objc open class PMAlertController: UIViewController {
    ...
    open var alertViewHeight: CGFloat = UIScreen.main.bounds.height < 568.0 ? 40 : 62
...
//        NotificationCenter.default.addObserver(self, selector: #selector(keyboardWillHide(_:)), name: UIResponder.NSNotification.Name.UIKeyboardWillHide, object: nil)
    }
...
//        NotificationCenter.default.removeObserver(self, name: UIResponder.NSNotification.Name.UIKeyboardWillHide, object: nil)
    ...
    @objc public convenience init(title: String, description: String, image: UIImage?, style: PMAlertControllerStyle) {
        self.init()
        guard let nib = loadNibAlertController(), let unwrappedView = nib[0] as? UIView
            else {
                return
        }
    ...
    @objc open func addAction(_ alertAction: PMAlertAction) {
        ...
        if alertActionStackView.arrangedSubviews.count > 2 || hasTextFieldAdded() {
            alertActionStackViewHeightConstraint.constant = alertViewHeight * CGFloat(alertActionStackView.arrangedSubviews.count)
            alertActionStackView.axis = .vertical
        } else {
            alertActionStackViewHeightConstraint.constant = alertViewHeight
