...
@IBDesignable class MMSlidingButton: UIView {
   ...
   var dragPointWidth: CGFloat = 140 {
        didSet {
            setStyle()
        }
    }
   ...
   var buttonCornerRadius: CGFloat = 18 {
    ...
    @IBInspectable var buttonUnlockedText: String   = "Order Placed".localizedString
    ...
    func setUpButton() {
        ...
        if !self.buttonText.isEmpty {
            ...
            self.dragPointButtonLabel = UILabel(frame: CGRect(x: 0, y: 0, width: self.frame.size.width - 10, height: self.frame.size.height))
            self.dragPointButtonLabel.textAlignment = .right
...
//          sender.setTranslation(CGPoint(x: 2, y: self.frame.size.height), in: self)
//        }
        
        if translatedPoint.x > (self.frame.maxX-self.dragPointWidth-24) {
          sender.setTranslation(CGPoint(x: (self.frame.maxX-self.dragPointWidth-24), y: self.frame.size.height), in: self)
        }
      }
